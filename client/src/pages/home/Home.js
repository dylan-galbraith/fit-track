...
const Home = ({ user, routines, exercises }) => {
  return(
    <main className="home">
      <h1 className="home__heading">Welcome, {user.firstName}!</h1>
      ...
      <article className="home__category">
        ...
        {exercises.filter(item => item.favourite).map(item => {
            return <Link key={item.id} to={`/exercises/${item.id}`} className={exercises.filter(item => item.favourite).indexOf(item)===0 ? "home__name home__name--top" : "home__name"}>{item.name}</Link>
          }
        )}
      </article>
    </main>
